...
//========================================================================
/*!
  \file    capture_hikmvcam.h
  \brief   C++ Interface: CaptureHikMvCam
  \author  Mark Huang, (C) 2022 mark_zjunlict@zju.edu.cn
*/
//========================================================================
#ifndef CAPTURE_HIKMVCAM_H
#define CAPTURE_HIKMVCAM_H
#include "captureinterface.h"
#include "MvCameraControl.h"
class CMvCamera;
class CaptureHikMvCam : public QObject, public CaptureInterface
{
    Q_OBJECT
public slots:
    void changed(VarType *);
protected:
    QMutex mutex;
...
protected:
    ...
    VarList * camera_params;

    // capture settings
    VarInt * v_capture_device;
    int m_device_index;
    // camera parameters
    ...
    MV_FRAME_OUT m_stFrame;

public:
    explicit CaptureHikMvCam(VarList * _settings= nullptr, int default_camera_id=0, QObject * parent=nullptr);
    ~CaptureHikMvCam() override;
    void mvcConnect(VarList *);
...
private:
    ...
    static AutoEnum toAutoEnum(const string & s){
        if(s == "manual")
            return manual_;
        else if(s == "once")
            return once_;
